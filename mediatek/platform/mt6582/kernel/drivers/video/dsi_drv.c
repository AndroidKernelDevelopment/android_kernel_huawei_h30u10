...
//merge Patch:ALPS.KK1.MP1.V2.42_P9, slove the problem cannot resume.
...
UINT32 DSI_dcs_read_lcm_reg_v3(UINT8 cmd, UINT8 *buffer, UINT8 buffer_size)
{
    UINT32 max_try_count = 5;
    UINT32 recv_data_cnt;
    unsigned int read_timeout_ms;
    ...
    static const long WAIT_TIMEOUT =  HZ/2;    // 2 sec  //Modified for ESD Check with out LCM
    long ret;
#endif
	if (DSI_REG->DSI_MODE_CTRL.MODE)
		return 0;
...
//       DISP_LOG_PRINT(ANDROID_LOG_INFO, "DSI", " DSI_RX_STA : 0x%x \n", DSI_REG->DSI_RX_STA);
	...
	if(dsi_log_on)
       ...
       else
       {
           ...
           //copy error report to buffer
           memcpy((void*)buffer,(void*)&read_data0.byte0, 3);
           memcpy((void*)(buffer+3),(void*)&read_data1, 4); 
       }
#endif
    //read lcm 0Ah reg , return packet_type = 0x11
   }while(packet_type != 0x1C && packet_type != 0x21 && packet_type != 0x22 && packet_type != 0x1A && packet_type != 0x11);
   /// here: we may receive a ACK packet which packet type is 0x02 (incdicates some error happened)
   /// therefore we try re-read again until no ACK packet
   /// But: if it is a good way to keep re-trying ???
#endif
...
void DSI_Config_DDPModule(void (*callback)(void *), void *param)
{
    ...
    if(is_early_suspended){
        printk("[DSI_Config_DDPModule] return directly because display driver is in early suspend mode\n");
